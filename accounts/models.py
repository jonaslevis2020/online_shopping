...
class MyAccoutManager(BaseUserManager):
    ...
    def create_user(self, first_name, last_name, user_name, email, password=None):
        if not email:
            raise ValueError('A user must have an email adress !!!')
        if not user_name:
            raise ValueError('A user must have a user_name !!!')
    ...
    def create_superuser(self, first_name, last_name, user_name, email, password=None):
        user = self.create_user(first_name, last_name, user_name, email, password)
        user.is_active      = True
        user.is_staff       = True
        user.is_admin       = True
